            ...
            Operand::Immediate(displacement) => {
                match instruction.length {
                    2 => {
                        ...
                        let displacement = *displacement as i8 as i32;
                        let base = cpu.registers.eip.wrapping_add(2);
                        cpu.registers.eip = base.wrapping_add(displacement as u32);
                    },
                    5 => {
                        // Near JMP (32-bit displacement)
                        let displacement = *displacement as i32;
                        let base = cpu.registers.eip.wrapping_add(5);
                        cpu.registers.eip = base.wrapping_add(displacement as u32);
                    },
...
mod tests {
    ...
    fn test_short_jmp_large_eip() {
        ...
        execute(&mut cpu, &mut memory, &instruction).unwrap();
        assert_eq!(cpu.registers.eip, 0x0001_0007);
    }
    ...
    #[test]
    fn test_near_jmp_high_address_negative_disp() {
        let mut cpu = CPU::new();
        let mut memory = Memory::default();

        cpu.registers.eip = 0x8000_0000;
        let instruction = Instruction {
            opcode: Opcode::JMP,
            dest: Some(Operand::Immediate((-2_i32) as u32)),
            src: None,
            length: 5,
        };

        execute(&mut cpu, &mut memory, &instruction).unwrap();
        assert_eq!(cpu.registers.eip, 0x8000_0003);
    }
